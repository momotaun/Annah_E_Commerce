...
import { resolveSafeRedirect } from "@/src/lib/loginRedirect";
...
function LoginForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // No captured "came from" page (e.g. the user opened /login directly)
      // falls back to the role's dashboard for staff accounts, or the
      // storefront home for an ordinary customer — not their profile page,
      // which is where this used to send everyone regardless of role.
      router.push(redirectTo ?? (user.role === "CUSTOMER" ? "/" : getHomeRouteForRole(user.role)));
    } catch (err) {
